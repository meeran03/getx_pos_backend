...
CREATE PROCEDURE [dbo].[EnrollStudent] @RollNumber varchar(7), @CourseID int, @Section varchar(1), @Semester varchar(15), @GPA float
AS
BEGIN
    SET NOCOUNT ON;
    IF @GPA < 2.5
    ...
    ELSE
    BEGIN
        INSERT [dbo].[Registration] ([Semester], [RollNumber], [CourseID], [Section], [GPA]) VALUES (@Semester, @RollNumber, @CourseID, @Section, @GPA);
        COMMIT;
    END
END